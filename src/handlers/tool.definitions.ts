...
export const TOOL_DEFINITIONS: McpTool[] = [
  ...
  // Contact tools
  {
    ...
    description: 'Search for contacts in Autotask by phone number or name. Two independent search modes - use only one per call. (1) phone: provide a complete phone number; the server extracts the last 4 digits, queries all three phone fields (phone, mobilePhone, alternatePhone) for candidates, then exact-matches the full number locally - pass only a real phone number, not names or other text. (2) searchTerm: a SINGLE token (last name OR first name OR email) that must appear inside one of firstName, lastName, or emailAddress — each field is checked separately, so a combined "First Last" string matches nothing. Use for name or email lookups when no phone number is available. To fetch a single contact by ID, use autotask_get_contact instead. Returns 25 results per page by default. Search results are internal work and not reported to the caller. When result count is 0, ask the caller once to spell the name letter-by-letter before retrying or pivoting. When the search returns no results matching the caller stated identity, the caller is not in this scope: proceed to UNVERIFIED INTAKE per kb_identity_sop. When the search returns a candidate, confirm silently against caller-provided details (name, company). On match, proceed to lock per existing flow. On mismatch, treat as no match.',
    inputSchema: {
      type: 'object',
      properties: {
        searchTerm: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        status: {
          type: 'number',
          description: 'Ticket status picklist integer ID. Always 1 (New) for new tickets in this instance.'
        },
        priority: {
          type: 'number',
