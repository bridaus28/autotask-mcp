...
export const TOOL_DEFINITIONS: McpTool[] = [
  ...
  },
  {
    ...
    description: 'Look up a technician by name and return their availability status. Available only when business_status is "open". Use this when a caller asks for a specific tech by first name. The status field is "available" when reachable, otherwise one of "not_available", "no_extension", "presence_unavailable", "ambiguous", "no_match". See kb_transfer_sop for outcome handling.',
    inputSchema: {
      type: 'object',
      properties: {
        searchTerm: {
